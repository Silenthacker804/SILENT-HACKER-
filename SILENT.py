...
{H2}\ \_\ \ \_\  \ \_\ \ \_\  \ \_____\  \ \_\    \ \_\   
 {H2}\/_/  \/_/   \/_/  \/_/   \/_____/   \/_/     \/_/   
         Made By {M2}SOFZY {P2}Coder""",width=80,style=f"{color_panel}"))
def info():
	prints(f"""{B2}╭─────────────────────╮{B2}╭───────────────╮
...
{B2}╰─────────────────────╯""")
	...
	print('')
	prints(nel(f'''\t{P2}[{H2}1{P2}] Sofzy method\t[{H2}2{P2}] Old school method\n\t[{H2}3{P2}] Don't Metode"''',width=80,style=f"{color_panel}")) 
	hc = input('[•] Pilih  : ')
	if hc in ['1','01']:
...
def mie():
	...
	print('') 
	prints(f' Mati/hidupkan Mode terbang 5000 id Jika Tidak Ada Hasil\n')
...
def sulap():
	...
	print(f'╰─ Mati/hidupkan Mode terbang 5000 id Jika Tidak Ada Hasil\n')
	prog = Progress(SpinnerColumn('clock'),TextColumn('{task.description}'),BarColumn(),TextColumn('{task.percentage:.0f}%'))
	des = prog.add_task('',total=len(id))
	with prog:
		with tred(max_workers=30) as pool:
...
def getkey():
    ...
    try:
        ...
        if yt in ["Y","y"]:
            os.system("xdg-open https://wa.me/+2348167676589?text=Assalamualaikum+bang+Meledak,+aku+mau+beli+scriptnya+tapi+yang+versi+premium.+Ini+lisensinya:%20"+crot+"+konfitmasi+nama+pembeli:%20"+namamu)
